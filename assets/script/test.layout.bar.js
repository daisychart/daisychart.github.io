...
var grouped = d3.daisy.layout.bar()
  ...
  .stacked(false)
  .axis({orient:'x', title:'x축이지요'}).axis('y')
  .legend(true)
  .grid(true)
  .label(true);
grouped.render();
...
var stackedNormalized = d3.daisy.layout.bar()
  .container('#content3')
  .data(dataset)
  .dimensions(['x', 'c'])
  .measures({field:'y', op:'mean'})
...
var mixed = d3.daisy.layout.bar() //measure가 2개 이상인 경우 경우
  .container('#content5')
  .data(dataset)
  .dimensions([{field:'c', order:'descending'}])
...
var mixStackedNormalized = d3.daisy.layout.bar() //measure가 2개 이상인 경우 경우
  .container('#content6')
  .data(dataset)
  .dimensions(['c'])
  .measures([{field:'x', op:'sum'}, {field:'y', op:'mean'}, {field:'z', op:'mean'}])
